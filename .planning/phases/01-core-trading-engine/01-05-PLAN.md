...
must_haves:
  truths:
    ...
    - "Orchestrator works identically with PaperExecutor and LiveExecutor -- same code path, same position logic, same P&L tracking (PAPR-02)"
  ...
  key_links:
    ...
    - from: "src/bot/orchestrator.py"
      ...
      via: "orchestrator delegates to position_manager which uses swappable executor (PAPR-02)"
      pattern: "position_manager.*open_position|close_position"
...
<task type="auto">
  ...
  <action>
    ...
    **PAPR-02 swappable executor verification** (add these tests to tests/test_orchestrator.py):
    - `test_orchestrator_works_with_paper_executor`: Create Orchestrator with a PositionManager backed by PaperExecutor. Call `open_position()` and `close_position()`. Verify position is opened, P&L is tracked, and funding settlement works. Assert the full lifecycle completes successfully.
    - `test_orchestrator_works_with_live_executor`: Create Orchestrator with a PositionManager backed by a mock LiveExecutor (same ABC). Call `open_position()` and `close_position()`. Verify identical behavior: same position opening logic, same P&L tracking calls, same funding settlement.
    - `test_executor_swap_produces_identical_behavior`: Parameterized test (pytest.mark.parametrize) that runs the SAME orchestrator scenario (open position -> simulate funding -> close position -> check P&L) with both PaperExecutor and mock LiveExecutor. Assert that both produce: (a) identical position_manager method calls, (b) identical pnl_tracker.record_open/record_close calls, (c) identical funding settlement triggers. This proves the orchestrator code path is truly executor-agnostic (PAPR-02).
    - Key assertion: The Orchestrator and PositionManager code does NOT branch on executor type -- all behavior differences are encapsulated in the Executor ABC implementation.
  </action>
  <verify>
    Run: `cd /Users/luckleineschaars/repos/funding-rate-arbitrage && python -m pytest tests/test_orchestrator.py -v`
    All tests pass.
    Run: `python -m pytest tests/test_orchestrator.py -v -k "paper_executor or live_executor or executor_swap"` (PAPR-02 specific tests pass)
  ...
  <done>
    Orchestrator wires all components and runs the main bot loop. P&L tracker simulates funding settlements. main.py creates the full dependency graph and starts the bot. All tests pass including full test suite. The bot can start in paper mode, monitor funding rates, and manually open/close positions with full P&L tracking. Swappable executor pattern verified end-to-end: Orchestrator produces identical behavior with PaperExecutor and LiveExecutor (PAPR-02).
  </done>
</task>
...
8. Orchestrator produces identical behavior with PaperExecutor and LiveExecutor -- swappable executor pattern verified end-to-end (PAPR-02)
</verification>
...
- Swappable executor verified: parameterized test proves identical orchestrator behavior with both PaperExecutor and LiveExecutor (PAPR-02)
</success_criteria>
